...
def vista_02_dashboard(driver):
    ...
    try:
        ...
        goto(driver, ruta)
        h1 = wait_visible(driver, (By.XPATH, "//h1[contains(text(),'Módulos principales')]"))
        shot = screenshot(driver, f"{vista}_positivo")
        record(vista, ruta, "POSITIVO", h1.is_displayed(),
               "Dashboard carga con titulo 'Módulos principales'", shot)
    except Exception as e:
        shot = screenshot(driver, f"{vista}_positivo_error")
        record(vista, ruta, "POSITIVO", False,                f"{type(e).__name__}: {e}", shot)
...
def _vista_en_construccion(driver, vista: str, ruta: str):
    """NEGATIVO sin sesion + POSITIVO verificando titulo 'Módulo en construcción'."""
    neg_sin_sesion(driver, vista, ruta)
    try:
        restore_session(driver)
        goto(driver, ruta)
        h1 = wait_visible(
            driver, (By.XPATH, "//h1[contains(@class,'construccion__title')]")
        )
        shot = screenshot(driver, f"{vista}_positivo")
        record(vista, ruta, "POSITIVO", h1.is_displayed(),
               f"Vista carga: '{h1.text}' (modulo pendiente en el frontend)", shot)
    except Exception as e:
        shot = screenshot(driver, f"{vista}_positivo_error")
        record(vista, ruta, "POSITIVO", False,                f"{type(e).__name__}: {e}", shot)
...
def vista_06_movimientos(driver):
    ...
    _vista_en_construccion(driver, "06_Movimientos_adaptada_produccion", "/produccion")
