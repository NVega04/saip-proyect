...
export default function Dashboard(): JSX.Element {
  ...
  const gridStyle: React.CSSProperties = {
    ...
    gap: cols === 1 ? "0.75rem" : "1rem",
  };

  // Icono más pequeño en móvil
  const iconSize = cols === 1 ? 32 : 40;
      ...
      <div style={gridStyle}>
        {modules.map((mod) => (
          <div
            ...
            style={{
              ...styles.card,
              padding: cols === 1 ? "1.5rem 1.25rem" : "2.5rem 2rem",
              flexDirection: cols === 1 ? "row" : "column",
              alignItems: cols === 1 ? "center" : "center",
              textAlign: cols === 1 ? "left" : "center",
              gap: cols === 1 ? "1rem" : "0.85rem",
            }}
            onClick={() => navigate(`/${mod.id}`)}
          >
            {/* Icono — contenedor fijo para el layout horizontal en móvil */}
            <div style={{ flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>
              {React.cloneElement(mod.icon, { width: iconSize, height: iconSize })}
            </div>
